    ...
    <main className="max-w-6xl mx-auto px-6 py-12">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {events.map((event, index) => (
          ...
          >
            <h2 className="text-2xl text-deep-green font-semibold">
              {event.title}
            </h2>

            <p className="mt-2 text-green">
              <strong className="text-deep-green">Date:</strong> {event.date}
            </p>

            <p className="mt-1 text-green">
              <strong className="text-deep-green">Location:</strong> {event.location}
            </p>
          </div>
        ))}
      </div>
